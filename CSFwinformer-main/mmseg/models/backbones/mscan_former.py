...
class MSCAN_Unet(nn.Module):
    def __init__(self, n_class=2, pretrained = True):
        super(MSCAN_Unet, self).__init__()
        self.n_class = n_class
        self.in_channel = 3
        # self.pretrained =pretrained
        config=[96, 192, 384, 768] # channles of convnext-tiny
    ...
    def forward(self, x):
        ...
        '''
         x128 torch.Size([2, 64, 128, 128])
         x64  torch.Size([2, 128, 64, 64])
         x32  torch.Size([2, 256, 32, 32])
         x16  torch.Size([2, 512, 16, 16])
        '''
        d32 = self.Up5(x16) #torch.Size([2, 256, 32, 32])
