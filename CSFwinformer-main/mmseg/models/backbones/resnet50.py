...
class FADC(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # FADC 卷积层的定义，和 Bottleneck 类似
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = AdaptiveDilatedConv(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=2):
        ...
        self.layer2 = self._make_layer(FADC, 128, layers[1], stride=2)
...
def debug_pam():
    pam_module = ResNet(Bottleneck, [3, 4, 6, 3])
    # B,C,H,W
